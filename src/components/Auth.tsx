...
export function Auth({ onAuthenticated }) {
  ...
  React.useEffect(() => {
    const checkExistingSession = async () => {
      try {
        ...
        console.log(
          "🔍 [AUTH MOUNT] Auth component mounted - checking for callbacks",
        );
        ...
        const oauthCallbackReady = sessionStorage.getItem(
          "eras-oauth-callback-ready",
        );
        const oauthExpectsGate = sessionStorage.getItem(
          "eras-oauth-expects-gate",
        );
        ...
        const isAuthCallback =
          isEmailVerificationFlow || isOAuthCallback || isPasswordResetFlow;
        ...
        if (!isAuthCallback) {
          console.log(
            "ℹ️ [AUTH MOUNT] Not an auth callback - skipping session check",
          );
          console.log(
            "ℹ️ [AUTH MOUNT] User should sign in manually via the form",
          );
          return;
        }
        ...
        if (isOAuthCallback && !isEmailVerificationFlow) {
          console.log(
            "✅ [AUTH MOUNT] OAuth callback detected - will process immediately",
          );
          console.log(
            "🚪 [AUTH MOUNT → ERAS GATE] OAuth redirect detected - will activate ErasGate",
          );
          ...
          console.log(
            "✅ [AUTH MOUNT] Email verification flow detected - checking for session",
          );
        }
        ...
        if (error) {
          ...
          console.log(
            "⚠️ [AUTH MOUNT] Verification link detected but session check failed",
          );
          console.log(
            "🔗 [AUTH MOUNT] This might indicate a Supabase configuration issue",
          );

          toast.error(
            "Email verification link processed, but session creation failed. Please try signing in manually.",
            {
              duration: 8000,
            },
          );
        ...
        if (session?.user) {
          console.log(
            "✅ [AUTH MOUNT] Found active session from email verification! Auto-logging in user:",
            session.user.email,
          );
          console.log(
            "📊 [AUTH MOUNT] Session user metadata:",
          ...
          try {
            ...
            if (!profile) {
              console.log(
                "⚠️ [AUTH MOUNT] No profile found, creating from metadata...",
              );
              ...
              try {
                ...
                console.log(
                  "✅ [AUTH MOUNT] Profile created successfully with display_name:",
                  displayName,
                );
                ...
                console.error(
                  "❌ [AUTH MOUNT] Failed to create profile:",
                  createError,
                );
            ...
            console.log(
              `🌙 [AUTH MOUNT → ANIMATION] Calling onAuthenticated for ${authType} with user data:`,
              userData,
            );
            console.log(
            ...
            console.log(
              `🎉 [AUTH MOUNT] Using fallback for ${authType}, calling onAuthenticated:`,
              userData,
            );
            ...
            try {
              ...
              if (oauthExpectsGate) {
                console.log(
                  "🚪 [AUTH MOUNT → ERAS GATE] OAuth callback confirmed (fallback) - clearing markers",
                );
                sessionStorage.removeItem("eras-oauth-flow");
                sessionStorage.removeItem("eras-oauth-timestamp");
                sessionStorage.removeItem("eras-oauth-expects-gate");
          ...
          if (
            ...
            toast.info(
              "Email verification link detected. Please sign in with your credentials.",
              {
                duration: 6000,
              },
            );
            ...
            try {
              ...
              if (email) {
                console.log(
                  "📧 [AUTH MOUNT] Pre-filling email from URL:",
                  email,
                );
                setFormData((prev) => ({
                  ...prev,
        ...
        if (
          ...
          toast.info(
            "Email verification detected. Please sign in with your credentials.",
            {
              duration: 6000,
            },
          );
  ...
  const canMakeRequest = (requestType, minIntervalMs = 2000) => {
    ...
    if (timeSinceLastRequest < minIntervalMs) {
      ...
      console.log(
        `🛡️ [DUPLICATE PREVENTION] ${requestType} request blocked - ${remainingWait}s remaining`,
      );
      toast.info(
        `Please wait ${remainingWait} second${remainingWait > 1 ? "s" : ""} before trying again`,
        {
  ...
  const handleVerifyEmail = async (e) => {
    ...
    try {
      ...
      if (data.user && data.session) {
        ...
        const capsulePendingTitle = sessionStorage.getItem(
          "capsule_pending_title",
        );

        console.log(
          "🌙 [EMAIL VERIFICATION → ANIMATION] Calling onAuthenticated after email verification",
        );
        console.log(
          "🌙 [EMAIL VERIFICATION → ANIMATION] NEW USER should see lunar eclipse animation",
        );
        onAuthenticated(
          {
            id: data.user.id,
        ...
        console.log(
          "🌙 [EMAIL VERIFICATION → ANIMATION] onAuthenticated called",
        );
        ...
        if (capsuleRedirect && viewToken) {
          ...
          try {
            ...
            const response = await fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-f9be53a7/api/view/${viewToken}`,
              {
                headers: {
            ...
            if (response.ok) {
              ...
              if (capsuleData.capsule?.id) {
                // Mark as received
                await DatabaseService.markCapsuleAsReceived(
                  capsuleData.capsule.id,
                );

                // Store flags for dashboard to show toast
                sessionStorage.setItem(
                  "capsule_just_received",
                  capsuleData.capsule.id,
                );
                sessionStorage.setItem(
                ...
                console.log(
                  "✅ Capsule marked as received, redirecting to dashboard",
                );
              }
            }
          } catch (error) {
          ...
          toast.success(
            "Email verified successfully! Opening your capsule... 🎉",
          );
      ...
      if (error) {
        ...
        if (
          ...
          toast.error("⏱️ Email Rate Limit Exceeded", {
            description:
              "For security, we can only send one verification email every 60 seconds. Please wait a moment and try again.",
            duration: 12000,
            action: {
              label: "Why?",
              onClick: () => {
                toast.info("Security Feature", {
                  description:
                    "Rate limits prevent spam and protect your account. Check your spam folder while you wait!",
                  duration: 8000,
                });
              },
            },
  ...
  const handleSignIn = async (e) => {
    ...
    try {
        ...
        if (
          ...
          try {
            const checkResponse = await fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-f9be53a7/api/auth/check-user-exists`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            console.log(
              "📦 📦 📦 FETCH COMPLETED! Response status:",
            ...
            console.warn(
              "⚠️ Unable to check user existence, showing generic helpful error",
            );
            console.log("🟡 🟡 🟡 SHOWING FALLBACK TOAST NOW!");
            toast.error("❌ Sign-In Failed", {
              description:
                "The email or password you entered is incorrect. Please double-check both fields.",
              duration: 10000,
              action: {
                label: "Forgot Password?",
                onClick: () => setCurrentView("forgot"),
                ...
                try {
                  ...
                  if (resendError) {
                    if (resendError.message.includes("already confirmed")) {
                      toast.error(
                        "Your email is already verified. Please try signing in again.",
                      );
                    } else if (
                      resendError.message
                        .toLowerCase()
                      ...
                      resendError.message.includes("rate_limit") ||
      ...
      if (data?.user) {
        ...
        if (rememberMe) {
          console.log(
            "💾 Remember Me enabled - configuring extended session (30 days)",
          );
          try {
            ...
            console.log(
              "✅ Extended session configured - you will stay signed in for 30 days",
            );
            ...
            console.warn(
              "Could not save Remember Me preferences:",
              storageError,
            );
          }
        } else {
          ...
          try {
            ...
            console.log(
              "🔒 Standard session configured - you will be signed out when you close the browser",
            );
          } catch (storageError) {
            console.warn(
              "Could not clear Remember Me preferences:",
              storageError,
            );
          }
        }
        ...
        try {
          ...
          console.error(
            "⚠️ Profile fetch failed, using fallback:",
            profileError,
          );
        ...
        try {
          ...
          console.log(
            "🎬 Calling onAuthenticated, keeping loading screen visible",
          );
          console.log(
          ...
          onAuthenticated(userData, data.session.access_token, {
            isFreshLogin: true,
          });
          console.log("✅ onAuthenticated callback completed successfully");
          console.log(
            '🌙 [SIGN-IN → ANIMATION] Check console for "LUNAR ECLIPSE ANIMATION WILL SHOW" message',
          );
      ...
      console.log(
        "🔄 Sign-in attempt completed (cleanup handled by individual branches)",
      );
    }
  };
  ...
  const handleSignUp = async (e) => {
    ...
    const loadingTimeout = setTimeout(() => {
      ...
      setIsLoading((currentLoading) => {
        ...
        if (currentLoading) {
          toast.error(
            "Sign-up is taking longer than expected. Please check your connection and try again.",
            {
              duration: 6000,
            },
          );
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          toast.error("Password is too weak or commonly used", {
            description:
              "Please choose a unique password with 8+ characters, letters, numbers, and special characters (!@#$)",
            duration: 8000,
          });
        } else if (
          error.message.toLowerCase().includes("email rate limit") ||
          ...
          toast.error("⏱️ Email Rate Limit Exceeded", {
            description:
              "For security, we can only send one verification email every 60 seconds. This prevents spam and protects your account. Please wait a moment and try again.",
            duration: 12000,
            action: {
              label: "Why?",
              onClick: () => {
                toast.info("Security Feature", {
                  description:
                    "Rate limits prevent automated bots from spamming accounts and help keep Eras secure for everyone. Thank you for your patience!",
                  duration: 8000,
                });
              },
            },
      ...
      if (data.user) {
        ...
        try {
          localStorage.setItem(
            `eras_account_created_${data.user.id}`,
            Date.now().toString(),
          );
          console.log(
        ...
        if (!data.session) {
          console.log(
            "📨 Email verification required - sending custom verification email via backend...",
          );

          try {
            const verificationResponse = await fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-f9be53a7/api/auth/send-verification-email`,
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (verificationResult.success) {
              ...
              toast.success(
                "Account created! Please check your email for verification link.",
                {
                  duration: 8000,
                  description:
                    'Check your inbox and spam folder for "Welcome to Eras!"',
                },
              );
            } else {
              console.error(
              ...
              toast.warning(
                "Account created, but verification email may be delayed.",
                {
                  duration: 8000,
          ...
          try {
            ...
            console.log(
              "✅ Profile created successfully with display_name:",
              displayName,
            );
            ...
            try {
              ...
              if (initResponse.ok) {
                ...
                console.warn(
                  "⚠️ Failed to initialize Time Novice title, but continuing",
                );
              }
            } catch (titleError) {
          ...
          // IMPORTANT: Pass the access token so pending capsules can be claimed
          console.log(
            "🌙 [SIGN-UP → ANIMATION] Calling onAuthenticated after successful sign-up",
          );
          console.log(
            "🌙 [SIGN-UP → ANIMATION] NEW USER should see lunar eclipse animation",
          );
          onAuthenticated(
            {
              id: data.user.id,
        ...
        toast.error(
          "Account creation failed - no user data received. Please try again.",
        );
        clearTimeout(loadingTimeout);
        setIsLoading(false);
      }
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      console.log(
        "🔐 Sending password reset request via custom server endpoint...",
      );
      ...
      if (!response.ok) {
        ...
        if (result.type === "rate_limit" || response.status === 429) {
          toast.error("⏱️ Email Rate Limit Exceeded", {
            description:
              "For security, we can only send one password reset email every 60 seconds. Please wait a moment and try again.",
            duration: 12000,
            action: {
              label: "Why?",
              onClick: () => {
                toast.info("Security Feature", {
                  description:
                    "Rate limits protect your account from automated attacks and ensure email delivery reliability.",
                  duration: 8000,
                });
              },
            },
        ...
        toast.success("Password reset email sent! 📧", {
          description:
            "Check your inbox for the reset link. It should arrive within seconds.",
          duration: 6000,
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes("provider is not enabled")) {
          ...
          toast.error(
            "Google OAuth configuration error. Please check your setup.",
            {
              duration: 8000,
              description:
                "Verify your Google Client ID and redirect URLs match",
            },
          );
        } else if (
          ...
          toast.error(
            "Google OAuth connection blocked. Please complete the setup.",
            {
              duration: 8000,
              description:
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          toast.error(
            "Failed to sign in with Apple. Please try email/password instead.",
          );
        }
        setIsLoading(false);
      }
          ...
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                The email should arrive within seconds. If you don't see it,
        ...
        <CardContent className="px-6">
          <Tabs value={currentView} onValueChange={setCurrentView}>
            <TabsList className="!w-full grid grid-cols-2 h-10 md:h-[34px] !flex-none !bg-transparent !p-0 gap-2">
              <TabsTrigger
                value="signin"
                className="!rounded-lg data-[state=active]:!bg-primary data-[state=active]:!text-primary-foreground data-[state=inactive]:!bg-muted"
              >
                Sign In
              </TabsTrigger>
              <TabsTrigger
            ...
            <TabsContent value="signin" className="space-y-4 mt-6">
              ...
              >
                ...
                {debugInfo.isIOS &&
                  debugInfo.attempts >= 2 &&
                  debugInfo.lastError && (
                    <div className="mt-4 space-y-2">
                      <div className="p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                        <p className="text-xs text-amber-800 dark:text-amber-200">
                          <strong>Having trouble?</strong> Try the button below
